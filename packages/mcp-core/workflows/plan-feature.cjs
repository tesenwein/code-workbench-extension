...
let plan = await agent(
  [
    ...
    contextBrief,
    '',
    'Return summary (what will be built and why), openQuestions (anything that needs a human decision before implementation), and tasks: a hierarchical breakdown where each task has title, description, priority, and subtasks (each with title, description). Order tasks and subtasks so earlier ones unblock later ones. Do not touch any code — this is planning only.',
  ].join('\\n'),
...
for (let round = 0; round < 2; round++) {
  ...
  plan = await agent(
    [
      ...
      '',
      'Return the full updated plan in the same shape: summary, openQuestions, tasks (with subtasks). Do not touch any code — this is planning only.',
    ].join('\\n'),
    { model: 'opus', phase: 'Synthesize', schema: PLAN_SCHEMA },
  )
}
