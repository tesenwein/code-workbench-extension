...
const PHASE_PROCEDURES = {
  ...
  implement: [
    ...
    IF_BLOCKED,
    "",
    'Work its subtasks tagged "plan-step" in `order` (lower first, unordered last; ties break by creation time — if there are none, work the task\'s description directly). Subtasks that share the same `order` AND are both flagged `parallel` form one wave: dispatch that whole wave together via concurrent subagents (one per subtask), wait for all of them to finish, then move to the next order. Subtasks at the same `order` that are NOT both `parallel`, or any subtask at a distinct `order`, are worked strictly one at a time in sequence. Mark each subtask in-progress before you start it and done when it passes. Run whatever lint, typecheck, and test scripts the project has; treat a failure as unfinished work, not a separate finding.',
    "",
  ...
  fix: [
    ...
    IF_BLOCKED,
    "",
    'List the subtasks tagged "review-finding" that were open when you started, and fix each one in `order` (lower first, unordered last; ties break by creation time). Findings that share the same `order` AND are both flagged `parallel` form one wave: dispatch that whole wave together via concurrent subagents (one per finding), wait for all of them to finish, then move to the next order. Findings at the same `order` that are NOT both `parallel`, or at a distinct `order`, are fixed strictly one at a time in sequence. Mark each finding in-progress before you start it and done once fixed. Re-run lint, typecheck, and tests at the end.',
    "",
