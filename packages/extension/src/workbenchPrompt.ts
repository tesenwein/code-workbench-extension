...
export const WORKBENCH_SYSTEM_PROMPT =
  ...
  '- COMMIT AFTER EVERY TASK: as soon as a task or subtask is marked "done" and its changes are checked (lint, typecheck, and existing tests — do NOT run the app end-to-end or invoke the /verify skill for this; the Review phase covers deeper validation), create a git commit covering exactly that task (Conventional Commits, subject referencing the task). One task = one commit, so progress stays clean and reviewable. Never bundle several finished tasks into one commit.\n' +
